...
const readDB = async <Type extends Record<any, any>>(
  dbPath: string,
): Promise<any> => {
...
const readEndpointsFromDB = async <
  Type extends Record<any, any>,
>(): Promise<Type> => readDB<Type>(ENDPOINTS_DB_FILE);
const readMetaDB = async <Type extends Record<any, any>>(): Promise<Type> =>
  readDB<Type>(COLLECTIONS_DB_FILE);
...
const readCollectionByName = async (collectionName: string): Promise<any> => {
  ...
  const collectionFilePath = data.filter((c) => c.name === collectionName)[0]
    .path;
